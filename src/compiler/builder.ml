...
let compile_to_assembly_code ast =
  ...
  | Wellformedness.IllFormed(strg) -> 
    let rec strconcat (str : string list) : string =
      match str with 
      |[] -> ""
      |n::r-> n ^ "\n" ^ strconcat r 
    in 
    let x: string = strconcat strg in
    raise (BuildFailure(
      sprintf "" ^ x
    ))
    (* failwith "
    _ is a string list 
    concat the string into a big string
    take the raise(buildfailure) from above to use that string
  " *)
